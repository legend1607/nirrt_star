...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='pointnet2tf', help='use pointnet2tf by default')
...
def main(args):
    ...
    valDataLoader = torch.utils.data.DataLoader(VAL_DATASET, batch_size=BATCH_SIZE, shuffle=False,
                                                pin_memory=True, num_workers=10, drop_last=False)
    ...
    # Model (专门 pointnet2tf)
    MODEL = importlib.import_module('pointnet_pointnet2.models.' + args.model)
    classifier = MODEL.get_model(NUM_CLASSES, coord_dim=TRAIN_DATASET.d, use_dir_head=True).cuda()
    criterion = MODEL.get_loss(use_dir_head=True).cuda()
    ...
    optimizer = torch.optim.Adam(classifier.parameters(), lr=args.learning_rate, betas=(0.9, 0.999),
                                 eps=1e-08, weight_decay=args.decay_rate) if args.optimizer == 'Adam' \
        else torch.optim.SGD(classifier.parameters(), lr=args.learning_rate, momentum=0.9)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        loss_sum = 0
        classifier.train()
        for _, batch in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            optimizer.zero_grad()
            ...
            seg_pred, trans_feat, dir_pred = classifier(points)
            seg_pred = seg_pred.contiguous().view(-1, NUM_CLASSES)
            target_flat = target.view(-1, 1)[:, 0]

            # loss 带 dir_pred
            loss = criterion(seg_pred, target_flat, trans_feat, weights, dir_pred=dir_pred, gt_dir=pc_xyz.cuda())
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            ...
            for _, batch in tqdm(enumerate(valDataLoader), total=len(valDataLoader), smoothing=0.9):
                pc_xyz_raw, pc_xyz, pc_features, pc_labels, token = batch
                ...
                loss = criterion(seg_pred_flat, target_flat, trans_feat, weights, dir_pred=dir_pred, gt_dir=pc_xyz.cuda())
                loss_sum += loss.item()

                pred_val = np.argmax(seg_pred.contiguous().cpu().numpy(), 2).reshape(-1)
                batch_label = target_flat.cpu().numpy()
            ...
            if best_optimal_path_IoU is None or optimal_path_IoU >= best_optimal_path_IoU:
                ...
                state = {'epoch': epoch, 'class_avg_iou': mIoU,
